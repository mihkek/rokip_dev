    ...
    <div class="content-wrapper mt-2">
        <section class="content">
            <div class="card card-outline card-primary">
                <form action="{{ isset($item) ? route('admin.devices.update',$item) : route('admin.devices.store') }}" method="POST" >
                    @isset($item) @method('PATCH') @endisset
                    @csrf

                    <div class="card-header">
                        <a href="{{ route('admin.devices.index') }}" class="text-primary mr-2" data-tooltip="tooltip" data-html="true" title="Вернуться к списку">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            <div class="col-md mb-3">
                                <div class="form-group">
                                    ...
                                    <div class="input-group date DATE" id="installation_at" data-target-input="nearest">
                                        <input type="text" name="installation_at" id="installation_at" class="form-control datetimepicker-input" data-target="#installation_at" value="{{ isset($request) ? $request->installation_at : (isset($item->installation_at) ? $item->installation_at->format('d.m.Y') : now()->format('d.m.Y')) }}" required/>
                                        <div class="input-group-append" data-target="#installation_at" data-toggle="datetimepicker">
                            ...
                            <div class="col-md mb-3">
                                ...
                                <select class="select2bs4 select2-hidden-accessible custom-select-sm" name="consumer_id" id="consumer_id" style="width: 100%;" tabindex="-1" aria-hidden="true" required>
                                    <option value="" selected disabled>Сделайте выбор</option>
                                    @foreach($consumers as $consumer)
                                        <option value="{{ $consumer->id }}" @if(isset($item) && $item->consumer_id == $consumer->id) selected @endif>{{ $consumer->title }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md mb-3">
                            ...
                            <div class="col-md mb-3">
                                @include('admin._include.form.input',['data'=>'meter_reading','type'=>'number','label'=>'Показания счетчика','required'=>true])
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md mb-3">
                            ...
                            <div class="col-md mb-3">
                                @include('admin._include.form.input',['data'=>'year_issue','type'=>'number','label'=>'Год выпуска','required'=>true])
                            </div>
                            <div class="col-md mb-3">
                                @include('admin._include.form.input',['data'=>'modification','label'=>'Модификация'])
                            </div>
                            <div class="col-md mb-3">
                                @include('admin._include.form.input',['data'=>'current','type'=>'number','label'=>'Сила тока, А'])
                            </div>
                            <div class="col-md mb-3">
                                @include('admin._include.form.input',['data'=>'voltage','type'=>'number','label'=>'Номинальное напряжение, В'])
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-12 mb-3">
                                @include('admin._include.form.input',['data'=>'address','label'=>'Адрес установки','required'=>true])
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-12 mb-3">
