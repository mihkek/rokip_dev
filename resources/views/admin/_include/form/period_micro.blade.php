...
<input
    ...
    value="{{ (isset($item) && isset($start) && $item->$start != null && isset($end) && $item->$end != null) ? Carbon::createFromTimestamp($item->$start, 'Europe/Moscow')->format('d.m.Y') . ' - ' . Carbon::createFromTimestamp($item->$end, 'Europe/Moscow')->format('d.m.Y') : null }}"
...
@push('scripts')
    <script>
        ...
        $('.DATAPICKER').daterangepicker({
            ...
            "startDate": @json(((isset($item) && isset($start) && $item->$start != null) ? Carbon::createFromTimestamp($item->$start, 'Europe/Moscow') : now()->addMonth(-1))->format('d.m.Y')),
            "endDate": @json(((isset($item) && isset($end) && $item->$end != null) ? Carbon::createFromTimestamp($item->$end, 'Europe/Moscow') : now())->format('d.m.Y')),
            "minDate": "10/01/2010",
